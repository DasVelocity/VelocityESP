...
if getgenv().velocity_ESP then
	return getgenv().velocity_ESP
end
...
local Library = {
	...
	PresetColors = {
		...
		LightRed = Color3.fromRGB(255, 102, 102),
...
function Library:Add(espSettings: ESPSettings)
	...
	if typeof(espSettings.Color) == "string" then
		espSettings.Color = Library.PresetColors[espSettings.Color] or Color3.new()
	end
	if typeof(espSettings.SurfaceColor) == "string" then
		espSettings.SurfaceColor = Library.PresetColors[espSettings.SurfaceColor] or Color3.new()
	end
	if typeof(espSettings.FillColor) == "string" then
		espSettings.FillColor = Library.PresetColors[espSettings.FillColor] or Color3.new()
	end
	if typeof(espSettings.OutlineColor) == "string" then
		espSettings.OutlineColor = Library.PresetColors[espSettings.OutlineColor] or Color3.new(1, 1, 1)
	end
	if typeof(espSettings.StrokeColor) == "string" then
		espSettings.StrokeColor = Library.PresetColors[espSettings.StrokeColor] or Color3.new(0, 0, 0)
	end
	if espSettings.Tracer and typeof(espSettings.Tracer.Color) == "string" then
		espSettings.Tracer.Color = Library.PresetColors[espSettings.Tracer.Color] or Color3.new()
	end
	if espSettings.Arrow and typeof(espSettings.Arrow.Color) == "string" then
		espSettings.Arrow.Color = Library.PresetColors[espSettings.Arrow.Color] or Color3.new()
	end
	...
	espSettings.StrokeColor = if typeof(espSettings.StrokeColor) == "Color3" then espSettings.StrokeColor else Color3.new(0, 0, 0);
	...
	InstancesLib.Create("UIStroke", {
		...
		Color3 = ESP.CurrentSettings.StrokeColor,
		Transparency = 0
	})
	...
	ESP.RenderThread = coroutine.create(function()
		while true do
			...
			if not success then
				task.defer(error, "Failed to render ESP:", errorMessage)
			end
